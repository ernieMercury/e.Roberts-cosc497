package 
{
	...
	public class Main extends Sprite 
	{
		...
		private function update( e:Event ):void
		{
			...
			for ( i = len - 1; i >= 0 ; i-- )
			{
				...
				if ( player.isColliding( enemy ) )
				{
					...
					{
						removeChild(enemies[i].enemyBMP);
		...
		private function initPlayer():void {
			...
			for (var k:int = 0; k < player.redMaps.length; k++) {
				addChild(player.redMaps[k]);
				player.redMaps[k].visible = false;
			}
			
			for (var j:int = 0; j < player.blueMaps.length; j++) {
				addChild(player.blueMaps[j]);
				player.blueMaps[j].visible = false;
			}
			
			if (player.isRed) player.redMaps[player.FRONT].visible = true;
			else player.blueMaps[player.FRONT].visible = true;
		}
